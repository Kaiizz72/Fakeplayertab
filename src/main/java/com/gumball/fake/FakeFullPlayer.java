...
public class FakeFullPlayer extends JavaPlugin implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("fakespawn")) {
            ...
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&aSpawned fake player!"));
            return true;
        } else if (cmd.getName().equalsIgnoreCase("fakeremove")) {
            removeFakes();
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cRemoved all fake players!"));
            return true;
        }
        return false;
    }
    ...
    // Fixed for Paper 1.21.1 + ProtocolLib 5.3+ (use List instead of EnumSet)
    private PacketContainer createPlayerInfoPacket(UUID uuid, String name, EnumWrappers.PlayerInfoAction action) {
        ...
        // 1.21.1 uses List for actions
        packet.getPlayerInfoActions().write(0, Collections.singletonList(action));
